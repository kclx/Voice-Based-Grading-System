...
- **智能姓名匹配**: 三级匹配策略处理同音字和发音偏差
  - 精确匹配 (杨洋 = 杨洋)
  - 拼音精确匹配 (yangyang = 杨洋)
  - 拼音模糊匹配 (yanyang ≈ 杨洋, 编辑距离 ≤ 1)
...
```python
ENERGY_THRESHOLD = 4000      # 声音能量阈值 (环境噪音大时增加)
PAUSE_THRESHOLD = 0.8        # 停顿时长 (秒)
...
```python
LEVENSHTEIN_THRESHOLD = 1    # 模糊匹配距离 (1=严格, 2=宽松)
```
